  ...
  if (newComponentsDiff.length > 0) {
    while (newComponentsDiff.length > 0) {
      ...
      try {
        ...
        newComponentsDiff = await newComponentsDiff.filter(
          (item) => item !== key,
        );
      } catch (error) {
        strapi.log.error(
          `Error registering component ${key}: ${error.message}`,
        );
...
const parseAndValidateComponent = async (
  filePath: string,
  strapi: Core.Strapi,
) => {
  try {
    ...
    if (
      ...
      strapi.stopWithError({
        message: `Invalid component structure in file: ${filePath}`,
      });
      return null;
    }
    ...
    strapi.stopWithError({
      message: `Error parsing file: ${filePath}: ${error.message}`,
    });
    return null;
  }
};
