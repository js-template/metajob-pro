...
async function createRelease() {
  ...
  if (errors) {
    throw new Error(
      `failed to create release for product ${KEYGEN_PRODUCT_ID}: ${JSON.stringify({ errors })}`,
    );
  }
...
async function uploadArtifactForRelease({
  ...
  if (errors) {
    throw new Error(
      `failed to create artifact for release ${releaseId}: ${JSON.stringify({ errors })}`,
    );
  }
...
async function getManifestForPackage() {
  ...
  if (errors) {
    throw new Error(
      `failed to retrieve manifest for ${PACKAGE_NAME}: ${JSON.stringify({ errors })}`,
    );
  }
